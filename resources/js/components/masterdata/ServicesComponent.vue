<template>
    <div class="p-2">
        ...
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item text-black" aria-current="page">Services</li>
        </ol>
        </nav>
                ...
                <div class="card" style="height: auto; ">
                    <div class="p-5">
                        <!-- <div class="form-group">
                            <label for="government-program" class="font-weight-bold">Government Program (Optional)</label>
                            <select  id="government-program" class="form-control" v-model="government_program_id" >
                                <option v-for="item in governmentprograms" :value="item.id">{{ item.name}}</option>
                            </select>
                          </div> -->
                        <div class="form-group">
                        ...
                        <div class="d-flex justify-content-end">
                            <div>
                                <div class="">
                                    <button v-if=" !isEdit" v-on:click="createRecord" type="button" class="btn btn-primary bg-blue border-none">Add Service</button>
...
export default{
  ...
  methods: {
    ...
    show() {
        this.isLoading = true;
        axios.get('/services/show').then(response=> {
    ...
    filterData() {
        this.isLoading = true;
        axios.post('/services/filterData', {
    ...
    edit(data) {
        ...
        this.government_program_id = data.row.government_program_id;
    },

    updateRecord() {
        axios.put('/services/update/' + this.id, {
            name: this.name,
            description: this.description,
            status : this.status,
    ...
    destroy(data) {
        if(confirm('Are you sure you want to delete this record?')) {
            ...
            axios.get('/services/destroy/' + data.row.id)
            .then(response => {
                // console.log(response);
                this.$fire({
    ...
    chanbeGovernmentProgram(){
        console.log(this.government_program_id);}
    },
    mounted() {
        this.isLoading = false;
        this.show();
