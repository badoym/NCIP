    ...
    <div class="p-2">
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="filter row align-items-center">
                        ...
                        <div class="col-md-2">
                            <button class="btn w-100 mt-3 bg-yellow" style="color:white!important" v-on:click="showAppliedProgram()" v-if="!showProgramApplied"> <i class="fas fa-window-maximize"></i>  Show Applied Programs</button>
                            <button class="btn w-100 mt-3 bg-yellow" style="color:white!important" v-on:click="showData()" v-if="showProgramApplied"> <i class="fas fa-window-maximize"></i>  Show Programs</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <!-- content -->
         <div style="height: 650px; overflow-y: scroll;" >
            <div class="row" v-show="!showProgramApplied">
                ...
                <div class="col-md-4" v-for="item in list">  
                    <div class="card p-3" style="box-shadow: rgba(0, 0, 0, 0.05) 0px 0px 0px 1px, rgb(209, 213, 219) 0px 0px 0px 1px inset;">
                        ...
                        <div class="card-footer p-3">
                            <button class="btn btn-outline-primary w-100" v-on:click="apply(item)">Enroll on this {{ item.government_program_id ? ' Program' : ' Service'}} </button>
                        </div>
                    </div>
                </div>
            </div>
            <div v-show="showProgramApplied">
                <div v-if="programApplied.length == 0" class="d-flex justify-content-center w-100">
                    <p class="text-center p-3">No Item to show</p>
                </div>
                <div v-else>
                    <p class=" p-3">Applied Services</p>
                    <div class="col-md-4" v-for="item in programApplied">  
                        <div class="card p-3" style="box-shadow: rgba(0, 0, 0, 0.05) 0px 0px 0px 1px, rgb(209, 213, 219) 0px 0px 0px 1px inset;">
                            <div class="card-header p-3 d-flex justify-content-between">
                                <div class="w-75">
                                    <p class="text-bold m-0 p-0">{{ item.name  }}</p>
...
export default{
  ...
  methods: {
    ...
    showData(){
        this.showProgramApplied = false;
        axios.get('/web_government_prog/show').then(response => {
    ...
    filterData(){
        ...
        axios.post('/web_government_prog/filter',  obj).then(response => {
            this.list = response.data.data;
        }).catch(error => {
            console.log(error);
        })
    ...
    apply(item){
        ...
        axios.get('/web_government_prog/apply/' + item.id)
        .then(response => {
    ...
    showAppliedProgram(){
        this.showProgramApplied = true;
        this.isLoading = true;
        axios.get('/web_government_prog/programapplied').then(response => {
            this.programApplied = response.data.data;
        }).catch(error => {
            this.$fire({
                title : 'Error',
    ...
    cancel(item){
        this.isLoading = true;
        axios.get('/web_government_prog/cancelappliedprogram/' + item.id).then(response => {
            this.$fire({
                title : 'Cancelled!',
                text : response.data.message,
                type : 'success',
